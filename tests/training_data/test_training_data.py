...
def test_balance_grouped_samples_preserves_data_below_minimum_target():
    grouped = {
        "gpt": list(range(10)),
        "single-sample": [99],
    }
    ...
    assert balanced == grouped
    assert target_count == 1


def test_balance_grouped_samples_rejects_invalid_minimum_target():
    try:
        balance_grouped_samples({"gpt": [1]}, minimum_target_count=0)
    except ValueError as exc:
        assert str(exc) == "minimum_target_count must be at least 1"
    else:
